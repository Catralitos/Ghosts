...
public class GhostController : MonoBehaviour
{
    ...
    public void StartMovement(Transform target)
    {
        ...
        if (this.rb.position.x != objective.position.x || this.rb.position.y != objective.position.y)
        {
            if (!Occupied(Vector2.right) && Vector2.right != -lastMovingDirection && (Vector2.Distance(this.rb.position + Vector2.right, objective.position) < distance || distance == 0))
            {
                distance = Vector2.Distance(this.rb.position + Vector2.right, objective.position);
                direction = Vector2.right;
            }
            if (!Occupied(Vector2.left) && Vector2.left != -lastMovingDirection && (Vector2.Distance(this.rb.position + Vector2.left, objective.position) < distance || distance == 0))
            {
                distance = Vector2.Distance(this.rb.position + Vector2.left, objective.position);
                direction = Vector2.left;
            }
            if (!Occupied(Vector2.up) && Vector2.up != -lastMovingDirection && (Vector2.Distance(this.rb.position + Vector2.up, objective.position) < distance || distance == 0))
            {
                distance = Vector2.Distance(this.rb.position + Vector2.up, objective.position);
                direction = Vector2.up;
            }
            if (!Occupied(Vector2.down) && Vector2.down != -lastMovingDirection && (Vector2.Distance(this.rb.position + Vector2.down, objective.position) < distance || distance == 0))
            {
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        ...
        if (node != null && (this.rb.position.x != objective.position.x || this.rb.position.y != objective.position.y))
        {
            if (!Occupied(Vector2.right) && Vector2.right != -lastMovingDirection && (Vector2.Distance(this.rb.position + Vector2.right, objective.position) < distance || distance == 0))
            {
                distance = Vector2.Distance(this.rb.position + Vector2.right, objective.position);
